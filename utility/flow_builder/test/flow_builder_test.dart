import 'package:flow_builder/src/flow_builder.dart';
...
void main() {
  group('FlowBuilder', () {
    group('constructor', () {
      ...
      test(
        'does not throw when state is null if controller is present',
        () async {
          expect(
            () => FlowBuilder(
              onGeneratePages: (dynamic _, List<Page<dynamic>> __) => [],
    ...
    testWidgets('throws FlutterError when context.flow is called '
...
''';
          ...
          home: FlowBuilder<int>(
            ...
            onGeneratePages: (state, pages) {
              ...
              return <Page<dynamic>>[
                ...
                if (state == 1)
                  MaterialPage<void>(
                    child: Scaffold(key: scaffoldKey, appBar: AppBar()),
                  ),
              ];
            },
          ),
          ...
          home: FlowBuilder<int>(
            ...
            onGeneratePages: (state, pages) {
              ...
              return <Page<dynamic>>[
                ...
                if (state == 1)
                  const MaterialPage<void>(child: Scaffold(key: scaffoldKey)),
              ];
            },
          ),
        ),
                        ...
                        body: TextButton(
                          ...
                          onPressed: () {
                            Navigator.of(context).push<void>(
                              MaterialPageRoute(
                                builder: (_) =>
                                    const Scaffold(key: scaffoldKey),
                              ),
                            );
                          },
                        ),
    ...
    testWidgets('navigation stack changes upon back pressed '
                  ...
                  return <Page<dynamic>>[
                    MaterialPage<void>(
                      child: TextButton(
                        ...
                        child: const Text('Button'),
                        onPressed: () =>
                            setState(() => controller = FlowController(1)),
                      ),
                    ),
                    if (state == 1)
                      const MaterialPage<void>(child: SizedBox(key: boxKey)),
            ...
            MaterialPage<void>(
              child: Builder(
                builder: (context) => PopScope(
                  canPop: false,
                  onPopInvokedWithResult: (_, _) => onPopCallCount++,
                  child: TextButton(
                    key: targetKey,
                    onPressed: () {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: FlowBuilder<int>(
            ...
            state: 0,
            onGeneratePages: (state, pages) {
              return <Page<dynamic>>[pageA, if (state == 1) pageB];
            },
          ),
        ),
      );
