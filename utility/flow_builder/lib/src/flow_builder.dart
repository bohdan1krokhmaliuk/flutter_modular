...
/// Signature for function which given as input flow state [T] will
/// output a new flow state [T].
///
/// It is used to compute the next flow state with [FlowController.update] and
/// [FlowController.complete]
typedef OnPopCallback<T> = void Function(T currentState, T previousState);
...
class FlowBuilder<T> extends StatefulWidget {
  ...
  /// Optional [OnPopCallback<T>] which is invoked when page is popped
  /// from navigation stack (e.g. via back gesture or back button).
  /// Receives current state and the previous state before the pop.
  final OnPopCallback<T>? onPop;
  ...
  /// Root router for proper check if the Flow is visible. It needed to respect
  /// SystemNavigation back on Android
  final GlobalKey<NavigatorState>? rootRouter;
...
class _FlowBuilderState<T> extends State<FlowBuilder<T>> {
  ...
  Future<bool> _pop() async {
    if (mounted) {
      ...
      if (mounted && !_canPop) {
        // call onPopInvoked when current flow `isCurrent`
        // and not the last in the stack
        if (Navigator.of(context).canPop() &&
            (widget.rootRouter?.currentWidget as Navigator?)?.pages.last.name ==
      ...
      child: _ConditionalPopScope(
        ...
        child: Navigator(
          ...
          onDidRemovePage: (page) {
            // Notify page removed (regardless of forward replace or pop)
            page.onPopInvoked(true, null);

            // Emit back analytics only for real pops (not internal replaces)
            if (!_isInternalStackUpdate && _history.length > 1) {
              final currentState = _history.last;
              final previousState = _history.elementAt(_history.length - 2);
...
class _InheritedFlowController<T> extends InheritedWidget {
  ...
  static FlowController<T> of<T>(BuildContext context) {
    final inheritedFlowController =
        context
                .getElementForInheritedWidgetOfExactType<
                  _InheritedFlowController<T>
                >()
                ?.widget
            as _InheritedFlowController<T>?;
